...
fn multiply_simple(n: usize, d1: u8, d2: u8) -> Vec<u8> {
    ...
    // ========================================================================
    // STEP 1: BUILD COEFFICIENT ARRAY
    // ========================================================================
    // n=4 → coeffs = [1, 2, 3, 4, 3, 2, 1]
    // n=8 → coeffs = [1, 2, 3, 4, 5, 6, 7, 8, 7, 6, 5, 4, 3, 2, 1]
    // ========================================================================
    let num_coeffs = 2 * n - 1;
    let mut coeffs: Vec<usize> = vec![0; num_coeffs];
    for i in 0..num_coeffs {
        coeffs[i] = std::cmp::min(i + 1, num_coeffs - i);
    }
    // AFTER STEP 1 (n=4): coeffs = [1, 2, 3, 4, 3, 2, 1]

    // ========================================================================
    // STEP 2: MULTIPLY EACH COEFFICIENT BY (d1 × d2)
    // ========================================================================
    // d1=4, d2=6 → product = 24
    // vals = coeffs × product = [24, 48, 72, 96, 72, 48, 24]
    // ========================================================================
    let product: u32 = (d1 as u32) * (d2 as u32);
    let mut vals: Vec<u32> = vec![0; num_coeffs];
    for i in 0..num_coeffs {
        vals[i] = (coeffs[i] as u32) * product;
    }
    // AFTER STEP 2 (n=4, d1=4, d2=6): vals = [24, 48, 72, 96, 72, 48, 24]

    // ========================================================================
    // STEP 3: CARRY PROPAGATION (convert multi-digit vals to single digits)
    // ========================================================================
    // vals = [24, 48, 72, 96, 72, 48, 24]
    // → result = [4, 0, 7, 3, 2, 6, 9, 2]
    // ========================================================================
    let mut result: Vec<u8> = Vec::new();
    let mut carry: u32 = 0;

    // STEP 3a: Process each val
    ...
    // STEP 3b: Handle remaining carry
    while carry > 0 {
        let digit = (carry % 10) as u8;
        carry = carry / 10;
        result.push(digit);
    }
    // AFTER STEP 3 (n=4, d1=4, d2=6): result = [4, 0, 7, 3, 2, 6, 9, 2]
